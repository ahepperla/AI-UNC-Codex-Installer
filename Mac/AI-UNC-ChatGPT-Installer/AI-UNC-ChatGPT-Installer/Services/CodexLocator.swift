...
final class CodexLocator: @unchecked Sendable {
    ...
    private func findDesktopAppPath() async -> String? {
        ...
        for directory in applicationDirectories {
            for appBundleName in Self.desktopAppBundleNames {
                let canonicalPath = "\(directory)/\(appBundleName)"
            ...
            if let appPath = applications
                .filter({ appName in
                    Self.desktopAppBundleNames.contains {
                        appName.localizedCaseInsensitiveCompare($0) == .orderedSame
                    }
                })
                .map({ "\(directory)/\($0)" })
        ...
        for appBundleName in Self.desktopAppBundleNames {
            guard let result = try? await runner.run(
                ...
                arguments: ["kMDItemFSName == '\(appBundleName)'c"]
        ...
        if let desktopAppPath {
            ...
            if let bundle = Bundle(url: URL(fileURLWithPath: desktopAppPath)),
               let version = bundle.object(forInfoDictionaryKey: "CFBundleShortVersionString") as? String {
                return "\(desktopDisplayName(for: desktopAppPath)) \(version)"
            }
    ...
    private func desktopDisplayName(for path: String) -> String {
        let appName = URL(fileURLWithPath: path)
            .deletingPathExtension()
            .lastPathComponent
        return appName.isEmpty ? "Desktop app" : "\(appName) Desktop"
    }
}
