...
struct FinishView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 20) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                CompletionDetailRow("API key handling configured.", systemImage: state.usePlaintextFallback ? "doc.plaintext" : "lock.shield.fill")
                CompletionDetailRow("UNC endpoint returned the expected response.", systemImage: "checkmark.seal.fill")
                CompletionDetailRow("Codex workspace: \(state.workspaceDirectoryPath)", systemImage: "folder.fill")
                ...
                HStack(alignment: .top, spacing: 12) {
                    ...
                    VStack(alignment: .leading, spacing: 4) {
                        Text("Next: Install or Open Codex")
                            .font(.headline)
                        Text("UNC configuration is finished. Codex Desktop is recommended for most users; the CLI is optional for Terminal workflows.")
                            .foregroundStyle(.secondary)
            ...
            VStack(alignment: .leading, spacing: 10) {
                ...
                Toggle(isOn: launchDesktopOnFinishBinding) {
                    Text("Open Codex Desktop when I click Finish")
                }
                .toggleStyle(.checkbox)
                .disabled(!state.codexDesktopAppAvailable || state.isBusy)
                .help(state.codexDesktopAppAvailable ? "Finish will open the installed Codex desktop app." : "Install Codex Desktop before using this option.")
    ...
    private var desktopActionSubtitle: String {
        ...
        return "Recommended for most users. Downloads and installs the Apple Silicon desktop app."
    }
    ...
    private var codexStatusText: String {
        if state.codexDesktopAppAvailable && state.codexCLIAvailable {
            return "Codex Desktop app and CLI detected."
        }
        if state.codexDesktopAppAvailable {
            return "Codex Desktop app detected."
        }
